...
def _make_pico_light_sparse_process_producer(cfg_ctrl: PicoLightSparseCtrlCfg):
    sync_cfg = cfg_ctrl.model_copy(update={"async_read": False})
    ctrl = PicoLightSparseCtrl(cfg_ctrl=sync_cfg)
...
class PicoLightSparseCtrl(Controller):
    ...
    def __init__(
        ...
        self._sdk_reader = None
        self._ee_pose_source = None
        ...
        if self.cfg_ctrl.async_read:
            ...
            self._async_worker = ProcessLatestOutputWorker(
                ...
                producer_factory=_make_pico_light_sparse_process_producer,
